...
%$for(name.literal)$\\ \author{$it.name$}$endfor$
%\author{$it.name.literal$}

%$-- $if(it.orcid)$~\orcidlink{$it.orcid$}$endif$$for(it.affiliations/first)$\\$it.name$$endfor$\\$if(it.email)$\href{mailto:$it.email$}{$it.email$}$endif$
